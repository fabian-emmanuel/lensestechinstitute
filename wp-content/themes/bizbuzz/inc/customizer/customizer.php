<?php
/**
 * bizbuzz: Customizer Main Class.
 *
 * @since 1.0.0
 */
if ( ! class_exists( 'Bizbuzz_Customizer' ) ) :

	class Bizbuzz_Customizer {
		...
		public function register( $wp_customize ) {
			do_action( 'bizbuzz_action_customize_register', $wp_customize );

			$this->customize = $wp_customize;

			$this->panels   = bizbuzz_get_customizer_panels();
			$this->sections = bizbuzz_get_customizer_sections();

			// Removing default header color option.
			$wp_customize->remove_control( 'header_textcolor' );

			$this->fields = bizbuzz_get_customizer_fields(); // Consist of settings and controls.
			$this->add_panels();
			$this->add_sections();
		...
		protected function add_control( $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'radio-image':
					$this->customize->add_control(
						new Bizbuzz_Radio_Image_Control(
							$this->customize,
							$field_id,
							$args
						)
				...
				case 'dropdown-posts':
					$this->customize->add_control(
						new Bizbuzz_Customize_Dropdown_Post_Control(
							$this->customize,
							$field_id,
							$args
						)
...
add_action( 'customize_register', array( Bizbuzz_Customizer::init(), 'register' ) );
