...
if ( ! function_exists( 'bizbuzz_default_breadcrumb' ) ) :
	...
	function bizbuzz_default_breadcrumb( $args = array() ) {
		...
		$text['category'] = __( 'Category : <em>%s</em>', 'bizbuzz' ); // phpcs:ignore
		$text['tax']      = __( 'Archive : <em>%s</em>', 'bizbuzz' ); // phpcs:ignore
		$text['search']   = __( 'Search results for: <em>%s</em>', 'bizbuzz' ); // phpcs:ignore
		$text['tag']      = __( 'Posts tagged :<em>%s</em>', 'bizbuzz' ); // phpcs:ignore
		$text['author']   = __( 'View all posts by <em>%s</em>', 'bizbuzz' ); // phpcs:ignore
		...
		if ( is_home() || is_front_page() ) {
			...
			<div id="crumbs" xmlns:v="http://rdf.data-vocabulary.org/#">
				...
				if ( is_category() ) {
					...
					?><span class="current"> <?php echo sprintf( $text['category'], single_cat_title( '', false ) ); ?></span><?php
				} elseif ( is_tax() ) {
					$thisCat = get_category( get_query_var( 'cat' ), false );
					if ( $thisCat->parent != 0 ) {
						$cats = get_category_parents( $thisCat->parent, true, $delimiter );
					...
					?><span class="current"><?php echo sprintf( $text['tax'], single_cat_title( '', false ) ); ?></span><?php
				} elseif ( is_search() ) {
					?><span class="current"><?php echo sprintf( $text['search'], esc_html( get_search_query() ) ); ?></span><?php
					...
					?><span class="current"><?php echo esc_html( get_the_time( 'd' ) ); ?></span><?php

				} elseif ( is_month() ) {
					echo sprintf( $link, esc_html( get_year_link( get_the_time( 'Y' ) ) ), esc_html( get_the_time( 'Y' ) ) ) . $delimiter; // phpcs:ignore 
					?><span class="current"><?php echo esc_attr( get_the_time( 'F' ) ); ?></span><?php

				} elseif ( is_year() ) {
					?><span class="current"><?php echo esc_attr( get_the_time( 'Y' ) ); ?></span><?php 
					...
					$post_type = get_post_type_object( get_post_type() );
					?><span class="current"><?php echo esc_html( $post_type->labels->singular_name ); ?></span><?php
					...
					if ( $showCurrent == 1 ) {
						echo $delimiter; 
						?><span class="current"><?php the_title(); ?></span><?php
					}
					...
					?>
						<span class="current"><?php echo sprintf( $text['tag'], esc_html( single_tag_title( '', false ) ) ); ?></span><?php

				} elseif ( is_author() ) {
					global $author;
					$userdata = get_userdata( $author );
					?><span class="current"><?php echo sprintf( $text['author'], esc_html( $userdata->display_name ) );?></span><?php

				} elseif ( is_404() ) {
					?><span class="current"><?php echo esc_html( $text['404'] ); ?></span><?php
				}
				...
				if ( get_query_var( 'paged' ) ) {
					...
					echo esc_html__( 'Page', 'bizbuzz' ) . ' ' . esc_html__( get_query_var( 'paged' ), 'bizbuzz' ); // phpcs:ignore 
					if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
						echo ')';
					}
				}
