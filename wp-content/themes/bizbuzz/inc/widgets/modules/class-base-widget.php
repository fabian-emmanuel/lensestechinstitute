<?php
/**
 * bizbuzz Widget Base Widget Class
 *
 * @link http://codex.wordpress.org/Widgets_API#Developing_Widgets
 * @since bizbuzz 1.0.0
 */

if ( ! class_exists( 'Bizbuzz_Widget' ) ) :

	class Bizbuzz_Widget extends WP_Widget {
			   ...
			   <p>
				...
				switch ( $field['type'] ) {
					...
					case 'description':
						...
						break;
					default:
							 echo esc_html__( 'Type Not Supported.', 'bizbuzz' );
						break;
				}
				?>
				 <?php if ( $field['type'] == 'checkbox' && isset( $field['label'] ) ) : ?>
		...
		public function get_html_dropdown_categories( $field ) {
			$dropdown = wp_dropdown_categories(
				array(
					...
					'echo'              => 0,
					'show_option_none'  => esc_html__( '&mdash; Select &mdash;', 'bizbuzz' ),
					'option_none_value' => '0',
		...
		public function get_html_dropdown_menus( $field ) {
			...
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( $field['id'] ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $field['id'] ) ); ?>">
				<option value="0"><?php echo esc_html__( '&mdash; Select &mdash;', 'bizbuzz' ); ?></option>
