...
class Bizbuzz_Footer_Widgets {
	...
	function init_widgets() {
		...
		if ( $this->active_widgets > 0 ) {
			add_action( 'bizbuzz_action_before_footer', array( $this, 'add_footer_widgets' ), 3 );
		}
	...
	function widgets_init_footer() {
		...
		for ( $i = 1; $i <= $this->max_widgets; $i++ ) {
			register_sidebar(
				array(
					'name'          => sprintf( __( 'Footer Widget %d', 'bizbuzz' ), $i ), // phpcs:ignore
	...
	function get_footer_widgets_content( $args ) {
		...
		$args = apply_filters( 'bizbuzz_filter_footer_widgets_args', $args );
		...
		for ( $i = 1; $i <= $number; $i++ ) {
			...
			$item_class  = apply_filters( 'bizbuzz_filter_footer_widget_class', '', $i );
